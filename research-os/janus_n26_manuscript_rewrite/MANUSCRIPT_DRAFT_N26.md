...
These results support a bounded combination claim: within a published-GSFE-anchored finite-contact model of Janus MoSSe, lateral-registry asymmetry can generate prepared-state unguided rectification and protocol-specific vector transport, while compact scaling, boundary-state selection, thermal mode mixing, and in-plane compliance define distinct limits of that response. The claim does not extend to a universal friction coefficient, a physical polarization-reversal law, arbitrary fully three-dimensional atomistic contacts, a Kelvin-scale thermal boundary, or a material-only nonlinear phase diagram.
