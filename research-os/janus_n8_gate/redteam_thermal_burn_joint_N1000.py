...
for burn in [10,30,60]:
    mu,mv=ens(N,20260919,.65,burn,*orbit)
    ...
    rng=np.random.default_rng(20260919+burn);B=10000;bm=np.empty((B,2))
    ...
    rows.append(dict(T=.65,burn=burn,n=N,mean_u=mean[0],se_u=mu.std(ddof=1)/math.sqrt(N),mean_v=mean[1],se_v=mv.std(ddof=1)/math.sqrt(N),hotelling_T2=T2,hotelling_crit95=crit,hotelling_excludes_zero=T2>crit,bootstrap_zero_d2=zd2,bootstrap_q95=q95,bootstrap_excludes_zero=zd2>q95))
df=pd.DataFrame(rows);df.to_csv('/mnt/data/janus_n8_gate/redteam_thermal_burn_joint_N1000.csv',index=False);print(df.to_string(index=False))
