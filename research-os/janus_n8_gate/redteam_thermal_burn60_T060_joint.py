...
T=.60;mu,mv=ens(N,20260919,T,60,*orbit)
...
rng=np.random.default_rng(20261060);B=5000;bm=np.empty((B,2))
...
out=pd.DataFrame([dict(T=T,burn=60,n=N,mean_u=mean[0],se_u=mu.std(ddof=1)/math.sqrt(N),mean_v=mean[1],se_v=mv.std(ddof=1)/math.sqrt(N),hotelling_T2=T2,hotelling_crit95=crit,hotelling_excludes_zero=T2>crit,bootstrap_zero_d2=zd2,bootstrap_q95=q95,bootstrap_excludes_zero=zd2>q95)])
out.to_csv('/mnt/data/janus_n8_gate/redteam_thermal_burn60_T060_joint.csv',index=False);print(out.to_string(index=False))
