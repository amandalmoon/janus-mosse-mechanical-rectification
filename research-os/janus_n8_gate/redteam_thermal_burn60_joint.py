...
mu,mv=ens(N,20260919,.65,60,*orbit)
X=np.column_stack([mu,mv]);np.save('/mnt/data/janus_n8_gate/redteam_burn60_T065_raw.npy',X)
mean=X.mean(0);Sx=np.cov(X,rowvar=False,ddof=1);Sinv=np.linalg.inv(Sx);p=2;n=N
T2=float(n*mean@Sinv@mean);crit=float(p*(n-1)/(n-p)*f.ppf(.95,p,n-p))
rng=np.random.default_rng(20260979);B=10000;bm=np.empty((B,2))
...
out=pd.DataFrame([dict(T=.65,burn=60,n=N,mean_u=mean[0],se_u=mu.std(ddof=1)/math.sqrt(N),mean_v=mean[1],se_v=mv.std(ddof=1)/math.sqrt(N),hotelling_T2=T2,hotelling_crit95=crit,hotelling_excludes_zero=T2>crit,bootstrap_zero_d2=zd2,bootstrap_q95=q95,bootstrap_excludes_zero=zd2>q95)])
out.to_csv('/mnt/data/janus_n8_gate/redteam_thermal_burn60_joint.csv',index=False);print(out.to_string(index=False))
