...
ens(2,1,.65,10,*orbit)
rows=[];N=1000
mu,mv=ens(N,20260919,.65,60,*orbit);rows.append(dict(T=.65,burn=60,start='orbit',n=N,mean_u=mu.mean(),se_u=mu.std(ddof=1)/math.sqrt(N),mean_v=mv.mean(),se_v=mv.std(ddof=1)/math.sqrt(N)))
df=pd.DataFrame(rows);df.to_csv('/mnt/data/janus_n8_gate/redteam_thermal_burn60_N1000.csv',index=False);print(df.to_string(index=False))
