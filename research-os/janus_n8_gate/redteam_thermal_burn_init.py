...
for T in [.65,.70]:
 for burn in [10,30,60]:
  for name,s0 in starts.items():
   mu,mv=ens(N,20260919,T,burn,*s0);rows.append(dict(T=T,burn=burn,start=name,n=N,mean_u=mu.mean(),se_u=mu.std(ddof=1)/math.sqrt(N),mean_v=mv.mean(),se_v=mv.std(ddof=1)/math.sqrt(N)))
df=pd.DataFrame(rows);df.to_csv('/mnt/data/janus_n8_gate/redteam_thermal_burn_init.csv',index=False);print(df.to_string(index=False))
