...
reps=[
 ('pinned',1.2875,40.,(0,0)),
 ('w11',1.9625,40.,(1,-1)),
 ('w12',2.5625,40.,(1,-2)),
 ('w23',3.0250,40.,(2,-3)),
 ('w34',3.4000,40.,(3,-4)),
 ('w27_high',3.6750,70.,(2,-7)),
]
...
pd.DataFrame(time_rows,columns=['name','F0','period','steps_per_cycle','u','v','m','n','lock_residual','expected_winding']).to_csv('/mnt/data/janus_n5_gate/n5_timestep_representatives.csv',index=False)
pd.DataFrame(orbit_rows,columns=['name','F0','period','x','y','vx','vy','u_onecycle','v_onecycle','m','n','fixed_RK4_closure','expected_winding']).to_csv('/mnt/data/janus_n5_gate/n5_relative_periodic_orbits.csv',index=False)
pd.DataFrame(floq_rows,columns=['name','F0','period','method','rtol','atol','max_step','solver_steps','mu1_abs','mu2_abs','mu3_abs','mu4_abs','closure']).to_csv('/mnt/data/janus_n5_gate/n5_floquet_multiplateau.csv',index=False)
pd.DataFrame(basin_rows,columns=['name','F0','period','du','dv','dvy','u','v','m','n','lock_residual','expected_winding']).to_csv('/mnt/data/janus_n5_gate/n5_basin_representatives.csv',index=False)
print('TIMESTEP')
