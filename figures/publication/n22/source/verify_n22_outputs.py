...
EXPECTED_MM={
 'Figure_1_credibility_registry_asymmetry_N22':(177.8,68.0),
 'Figure_2_prepared_state_depinning_N22':(177.8,124.0),
 'Figure_3_same_spectrum_mechanism_N22':(177.8,108.0),
 'Figure_4_compact_elastic_robustness_N22':(177.8,120.0),
 'Figure_5_boundary_registry_switching_N22':(177.8,121.0),
 'Figure_6_vector_mode_locking_N22':(177.8,124.0),
 'Figure_7_thermal_hierarchy_N22':(177.8,102.0),
}
...
def main():
    ...
    for stem,(wmm,hmm) in EXPECTED_MM.items():
        ...
        if args.require_arial:
            ...
            if any(('Arimo' in name or 'Liberation' in name) for name in fonts): raise AssertionError(f'{stem}: fallback font leaked: {sorted(fonts)}')
        doc.close()
    ...
    (out/'N22_FIGURE_SHA256SUMS.txt').write_text(''.join(f'{h}  {n}\n' for h,n in sorted(sums,key=lambda z:z[1])),encoding='utf-8')
    print('N22 FIGURE EXPORT PREFLIGHT: PASS')
    return 0
