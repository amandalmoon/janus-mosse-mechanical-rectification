...
def discrete_gray(n_levels: int, reverse: bool = False):
    base = plt.get_cmap('Greys_r' if reverse else 'Greys')
    return matplotlib.colors.ListedColormap(base(np.linspace(0.08, 0.88, n_levels)))
...
def make_figure(release: Path, out: Path) -> None:
    ...
    ax_c.plot(q.F0, q.m, marker='o', ms=4.0, mfc=DARK, mec=DARK, color=DARK, lw=1.05, label=r'$m$')
    ax_c.plot(q.F0, q.n, marker='s', ms=3.8, mfc='white', mec=MID, color=MID, lw=1.00, ls='--', label=r'$n$')
