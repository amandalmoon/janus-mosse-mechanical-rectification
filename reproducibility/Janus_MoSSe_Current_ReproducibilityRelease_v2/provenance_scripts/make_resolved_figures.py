...
ns={}
code=open('/mnt/data/janus_resolve/matched_symmetry_audit.py').read().split('rows=[]')[0]
exec(code,ns)
centered, sym, inv = ns['centered'], ns['sym'], ns['inv']
A=np.column_stack([J.A1,J.A2])
OUT=Path('/mnt/data/janus_resolve/figures_resolved'); OUT.mkdir(exist_ok=True)
...
ad=pd.read_csv('/mnt/data/janus_resolve/asymmetry_global_check.csv')
...
md=pd.read_csv('/mnt/data/janus_resolve/matched_symmetry_final.csv'); G=md[md.min_id.astype(str)=='GLOBAL']
...
bs=pd.read_csv('/mnt/data/janus_resolve/branch_family_summary.csv')
...
ss=pd.read_csv('/mnt/data/janus_resolve/shape_scaling_extended.csv'); sm=pd.read_csv('/mnt/data/janus_resolve/shape_scaling_metrics.csv')
fig,axs=plt.subplots(2,2,figsize=(7.2,5.0),constrained_layout=True)
...
tr=pd.read_csv('/mnt/data/janus_resolve/triangle_ground_switch.csv'); ew=pd.read_csv('/mnt/data/janus_resolve/edge_weight_ground_switch.csv')
fig,axs=plt.subplots(2,2,figsize=(7.2,5.0),constrained_layout=True)
ax=axs[0,0]
...
vm=pd.read_csv('/mnt/data/janus_resolve/vector_mode_map_F0_period.csv'); fl=pd.read_csv('/mnt/data/janus_resolve/floquet_conditioning_check.csv')
...
th=pd.read_csv('/mnt/data/janus_resolve/free_thermal_cluster_ci_canonical.csv')
